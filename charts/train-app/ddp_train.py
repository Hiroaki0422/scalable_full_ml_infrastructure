...
def train(train_dataset, params, batch_size=32):
    ...
    validation = TimeSeriesDataSet.from_dataset(
        training, val_data, predict=True)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        # Save the model with epoch and validation loss
        ...
        # Save only the best model (change to save more if needed)
        save_top_k=1
    )

    # Create the EarlyStopping callback
    early_stopping = EarlyStopping(
        # Metric to monitor (can also be 'val_accuracy', etc.)
        monitor='val_loss',
    ...
    trainer = Trainer(
        ...
        strategy="ddp",                    # <-- enables DistributedDataParallel
        devices="auto",                    # or set =2 to force 2 GPUs/CPUs
        enable_model_summary=True,
        gradient_clip_val=gradient_clip_val,
        callbacks=[checkpoint_callback,
                   early_stopping, MLflowLoggingCallback()],
    )
